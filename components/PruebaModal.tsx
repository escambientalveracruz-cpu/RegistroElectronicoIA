...
interface PruebaModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSave: (prueba: Omit<Prueba, 'id' | 'cursoLectivoId' | 'periodoNombre' | 'subject'>) => void;
    pruebaExistente: Prueba | null;
    porcentajeTotal: number;
    porcentajeYaAsignado: number;
}

const PruebaModal: React.FC<PruebaModalProps> = ({ isOpen, onClose, onSave, pruebaExistente, porcentajeTotal, porcentajeYaAsignado }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            setNombre(pruebaExistente?.nombre || '');
            setPorcentaje(pruebaExistente?.porcentaje || 0);
            setPuntosTotales(pruebaExistente?.puntosTotales || 0);
        }
    }, [isOpen, pruebaExistente]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const porcentajeAjustado = pruebaExistente ? porcentajeYaAsignado - pruebaExistente.porcentaje : porcentajeYaAsignado;
    ...
    const porcentajeAjustadoParaValidacion = pruebaExistente ? porcentajeYaAsignado - pruebaExistente.porcentaje : porcentajeYaAsignado;
                ...
                <form onSubmit={handleSubmit}>
                    <div className="p-6">
                        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">
                            {pruebaExistente ? 'Editar Prueba' : 'Añadir Nueva Prueba'}
                        </h2>
                        ...
                        <div className="space-y-4">
                            <div>
                                <label htmlFor="nombrePrueba" className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-2">Nombre de la Prueba</label>
                                <input id="nombrePrueba" type="text" value={nombre} onChange={e => setNombre(e.target.value)} className="w-full form-input" required />
                            </div>
                    ...
                    <div className="flex justify-end items-center gap-4 p-4 bg-slate-100 dark:bg-slate-700/50 rounded-b-lg">
                        ...
                        <button type="submit" className="px-6 py-2 bg-indigo-500 text-white font-semibold rounded-md hover:bg-indigo-600">Guardar Prueba</button>
                    </div>
                </form>
                 <style>{`
