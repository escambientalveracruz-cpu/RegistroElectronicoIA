...
import type { CursoLectivo, Estudiante, ConfiguracionPrueba, Prueba, CalificacionPrueba } from '../types';
import { EstadoEstudiante } from '../types';
import PruebaModal from './PruebaModal';
import ComentarioAIPruebaModal from './ComentarioAIPruebaModal';

interface PruebasProps {
    cursoActivo: CursoLectivo | null;
    estudiantes: Estudiante[];
    configuraciones: ConfiguracionPrueba[];
    onSaveConfiguracion: (config: ConfiguracionPrueba) => void;
    pruebas: Prueba[];
    calificaciones: CalificacionPrueba[];
    onSavePrueba: (prueba: Prueba) => void;
    onDeletePrueba: (pruebaId: string) => void;
    onSaveCalificacion: (calificacion: CalificacionPrueba) => void;
}

const Pruebas: React.FC<PruebasProps> = ({ cursoActivo, estudiantes, configuraciones, onSaveConfiguracion, pruebas, calificaciones, onSavePrueba, onDeletePrueba, onSaveCalificacion }) => {
    ...
    const [selectedCalificacionData, setSelectedCalificacionData] = useState<{ estudiante: Estudiante; prueba: Prueba; calificacion: CalificacionPrueba } | null>(null);
    ...
    const activePruebas = useMemo(() => {
        if (!cursoActivo || !selectedPeriodoName || !selectedSubject) return [];
        return pruebas.filter(p => p.cursoLectivoId === cursoActivo.id && p.periodoNombre === selectedPeriodoName && p.subject === selectedSubject);
    }, [pruebas, cursoActivo, selectedPeriodoName, selectedSubject]);
    
    const porcentajeAsignado = useMemo(() => activePruebas.reduce((sum, p) => sum + p.porcentaje, 0), [activePruebas]);
    
    const calificacionesMap = useMemo(() => {
        const map = new Map<string, CalificacionPrueba>();
    ...
    const handleToggleEnabled = () => {
        ...
        const newConfig: ConfiguracionPrueba = {
    ...
    const handleOpenPruebaModal = (prueba: Prueba | null) => {
        setEditingPrueba(prueba);
        setIsPruebaModalOpen(true);
    };
    
    const handleOpenComentarioAIModal = (estudiante: Estudiante, prueba: Prueba, calificacion: CalificacionPrueba) => {
        setSelectedCalificacionData({ estudiante, prueba, calificacion });
        setIsComentarioAIModalOpen(true);
    };

    const handleSavePrueba = (pruebaData: Omit<Prueba, 'id' | 'cursoLectivoId' | 'periodoNombre' | 'subject'>) => {
        if (!cursoActivo || !selectedPeriodoName || !selectedSubject) return;
        const newPrueba: Prueba = {
            id: editingPrueba?.id || `prueba_${Date.now()}`,
            cursoLectivoId: cursoActivo.id,
            periodoNombre: selectedPeriodoName,
            subject: selectedSubject,
            ...pruebaData
        };
        onSavePrueba(newPrueba);
        setIsPruebaModalOpen(false);
        setEditingPrueba(null);
    };

    const handleCalificacionChange = (estudianteId: string, prueba: Prueba, puntos: string) => {
        const puntosNum = puntos === '' ? null : parseFloat(puntos);
        if (puntosNum !== null && (isNaN(puntosNum) || puntosNum < 0 || puntosNum > prueba.puntosTotales)) {
            return;
        }

        const newCalificacion: CalificacionPrueba = {
            id: `${prueba.id}-${estudianteId}`,
            pruebaId: prueba.id,
            estudianteId,
    ...
    const handleSetNoEntregado = (estudianteId: string, prueba: Prueba) => {
        const calificacionId = `${prueba.id}-${estudianteId}`;
    ...
    return (
        <div className="max-w-full mx-auto space-y-6">
            ...
            <div className="p-6 bg-white dark:bg-slate-700 rounded-xl shadow-lg">
                <h1 className="text-3xl font-bold text-slate-900 dark:text-white border-b border-slate-200 dark:border-slate-600 pb-4 mb-4">Calificación de Pruebas</h1>
            ...
            <>
                ...
                <div className="p-6 bg-white dark:bg-slate-700 rounded-xl shadow-lg space-y-4">
                    <div className="flex justify-between items-center pb-4 border-b border-slate-200 dark:border-slate-600">
                        <div>
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white">Evaluar Pruebas</h3>
                        ...
                        >
                            <span className="sr-only">Activar evaluación de pruebas</span>
                            <span
                                aria-hidden="true"
                    ...
                    <div className={`space-y-4 pt-4 transition-opacity duration-300 ${!isEnabled ? 'opacity-40 pointer-events-none' : 'opacity-100'}`}>
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            <div>
                                ...
                                <p className="text-sm text-slate-500 dark:text-slate-400">Valor total de todas las pruebas.</p>
                            </div>
                                ...
                                <thead className="text-xs text-slate-700 uppercase bg-slate-100 dark:bg-slate-600 dark:text-slate-200">
                                    <tr>
                                            ...
                                            <React.Fragment key={prueba.id}>
                                                <th className="px-2 py-3 border-l dark:border-slate-500 min-w-[170px]">
                                                    <div className="flex flex-col items-center">
                                                        ...
                                                        <span className="font-normal text-slate-500 dark:text-slate-400">({prueba.puntosTotales} pts)</span>
                                                    </div>
                                                </th>
                                        ...
                                        <th className="px-4 py-3 border-l dark:border-slate-500 min-w-[150px]">
                                            <button onClick={() => handleOpenPruebaModal(null)} className="px-3 py-1 text-xs bg-indigo-500 text-white rounded-md hover:bg-indigo-600">Añadir Prueba</button>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {activeStudents.map(student => {
                                        ...
                                        return (
                                            <tr key={student.id} className="border-b dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-600/50">
                                                ...
                                                {activePruebas.map(prueba => {
                                                    const calificacion = calificacionesMap.get(`${prueba.id}-${student.id}`);
                                                    const puntosObtenidos = calificacion?.puntosObtenidos;
                                                    const noEntregado = calificacion?.noEntregado;
                                                    const porcentajeObtenido = (puntosObtenidos !== null && puntosObtenidos !== undefined && prueba.puntosTotales > 0)
                                                        ? (puntosObtenidos / prueba.puntosTotales) * prueba.porcentaje
                                                        : 0;
                                                            ...
                                                            <td className="px-2 py-2 border-l dark:border-slate-500">
                                                                <div className="flex items-center justify-center gap-1.5">
                                                                    <input
                                                                        ...
                                                                        onChange={(e) => handleCalificacionChange(student.id, prueba, e.target.value)}
                                                                        ...
                                                                        max={prueba.puntosTotales}
                                                                        min={0}
                                                                        disabled={noEntregado}
                                                                    />
                                                                    <div className="flex flex-col items-center justify-center gap-1">
                                                                        <button
                                                                            onClick={() => handleSetNoEntregado(student.id, prueba)}
                                                                        ...
                                                                        {calificacion && puntosObtenidos !== null && puntosObtenidos !== undefined && !noEntregado && (
                                                                            <button
                                                                                onClick={() => handleOpenComentarioAIModal(student, prueba, calificacion)}
                                    ...
                                    <tr className="bg-slate-50 dark:bg-slate-600/50">
                                        ...
                                        {activePruebas.map(prueba => (
                                            <td key={prueba.id} colSpan={2} className="px-2 py-2 border-x dark:border-slate-500">
                                                <div className="flex justify-center items-center gap-2">
                                                    <button onClick={() => handleOpenPruebaModal(prueba)} className="p-1.5 rounded-md hover:bg-slate-200 dark:hover:bg-slate-500" title="Editar Prueba"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                                                    <button onClick={() => onDeletePrueba(prueba.id)} className="p-1.5 rounded-md hover:bg-red-200 dark:hover:bg-red-800 text-red-500" title="Eliminar Prueba"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                                                </div>
                                            </td>
                                        ))}
                                        <td colSpan={2}></td>
