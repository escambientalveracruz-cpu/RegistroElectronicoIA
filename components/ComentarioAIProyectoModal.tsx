...
import type { Estudiante, Proyecto, CalificacionProyecto } from '../types';
import { generateContent } from '../lib/gemini-client';

interface ComentarioAIProyectoModalProps {
  isOpen: boolean;
  onClose: () => void;
  estudiante: Estudiante;
  proyecto: Proyecto;
  calificacion: CalificacionProyecto;
}

const ComentarioAIProyectoModal: React.FC<ComentarioAIProyectoModalProps> = ({ isOpen, onClose, estudiante, proyecto, calificacion }) => {
  ...
  const generateComment = async () => {
    if (!estudiante || !proyecto || !calificacion) return;
    ...
    try {
      const prompt = `Como asistente de un docente, redacta un borrador de comentario constructivo y personalizado para un estudiante basado en su calificación en un proyecto.

      Datos:
      - Nombre del Estudiante: ${estudiante.nombre}
      - Nombre del Proyecto: ${proyecto.nombre}
      - Calificación Obtenida: ${calificacion.puntosObtenidos} de ${proyecto.puntosTotales} puntos.
      - Porcentaje del Proyecto: ${proyecto.porcentaje}%
        ...
        <div className="flex-shrink-0 p-5 flex items-start justify-between border-b border-slate-200 dark:border-slate-700">
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                Para: {`${estudiante.nombre} ${estudiante.primerApellido}`} en "{proyecto.nombre}"
              </p>
            </div>
            <button 
              type="button" 
...
export default ComentarioAIProyectoModal;