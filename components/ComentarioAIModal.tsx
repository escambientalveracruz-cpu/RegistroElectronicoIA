...
import type { Estudiante, Tarea, CalificacionTarea } from '../types';
import { generateContent } from '../lib/gemini-client';

interface ComentarioAIModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const ComentarioAIModal: React.FC<ComentarioAIModalProps> = ({ isOpen, onClose, estudiante, tarea, calificacion }) => {
  ...
  const generateComment = async () => {
    if (!estudiante || !tarea || !calificacion) return;
    ...
    try {
      const prompt = `Como asistente de un docente, redacta un borrador de comentario constructivo y personalizado para un estudiante basado en su calificación en una tarea.

      Datos:
      - Nombre del Estudiante: ${estudiante.nombre}
      - Nombre de la Tarea: ${tarea.nombre}
      - Calificación Obtenida: ${calificacion.puntosObtenidos} de ${tarea.puntosTotales} puntos.
      - Porcentaje de la Tarea: ${tarea.porcentaje}%
        ...
        <div className="flex-shrink-0 p-5 flex items-start justify-between border-b border-slate-200 dark:border-slate-700">
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                Para: {`${estudiante.nombre} ${estudiante.primerApellido}`} en "{tarea.nombre}"
              </p>
            </div>
            <button 
              type="button" 
...
export default ComentarioAIModal;